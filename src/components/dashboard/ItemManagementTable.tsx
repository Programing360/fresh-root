...
export default function ItemManagementTable({
  ...
  const [activeDeletionTarget, setActiveDeletionTarget] = useState<
    string | null
  >(null);
  ...
  const handleAvailabilityToggle = async (item: Product) => {
    ...
    try {
      ...
      setCatalog((prev) =>
        prev.map((p) =>
          p._id === item._id ? { ...p, availability: newAvailability } : p,
        ),
      );
      toast.success(
        `Marked as ${newAvailability ? "Available" : "Unavailable"}`,
      );
    } catch (err) {
      if (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-neutral-200/80 bg-white dark:border-neutral-900 dark:bg-neutral-950/40">
        ...
        <div className="flex items-center gap-2 border dark:border-neutral-800 p-1 rounded-xl w-max bg-neutral-50 dark:bg-neutral-900/40">
          <button
            ...
            className={`p-2 rounded-lg transition-all ${layoutMode === "table" ? "bg-white dark:bg-neutral-800 text-emerald-500 shadow-sm" : "text-neutral-400"}`}
          >
            <List size={15} />
          </button>
          <button
            onClick={() => setLayoutMode("grid")}
            className={`p-2 rounded-lg transition-all ${layoutMode === "grid" ? "bg-white dark:bg-neutral-800 text-emerald-500 shadow-sm" : "text-neutral-400"}`}
          >
            <LayoutGrid size={15} />
          </button>
        </div>
      ...
      <AnimatePresence mode="wait">
        {parsedCatalog.length === 0 ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="p-12 text-center border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-2xl flex flex-col items-center justify-center gap-3"
          >
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="overflow-x-auto rounded-2xl border border-neutral-200/80 bg-white dark:border-neutral-900 dark:bg-neutral-950/40 shadow-sm"
          >
                  ...
                  >
                    <td className="p-4">
                      <img
                        src={item.image}
                        alt=""
                        className="h-10 w-10 rounded-lg object-cover border dark:border-neutral-800"
                      />
                    </td>
                    ...
                    <td className="p-4 text-right space-x-1.5">
                      ...
                      <button
                        onClick={() => setActiveDeletionTarget(item._id)}
          ...
          >
            {parsedCatalog.map((item) => (
              <div
                key={item._id}
                className="rounded-2xl border border-neutral-200/80 bg-white p-4 dark:border-neutral-900 dark:bg-neutral-950/40 shadow-sm flex flex-col justify-between hover:shadow-md transition-all group"
              >
                <div>
                ...
                <div className="flex items-center justify-between mt-6 pt-3 border-t dark:border-neutral-900">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => setViewTarget(item)}
                      className="p-2 text-neutral-400 hover:text-neutral-900 dark:hover:text-white border dark:border-neutral-800 rounded-lg"
                    >
                      <Eye size={13} />
                    </button>
                    <button
                    ...
                    <button
                      onClick={() => setActiveDeletionTarget(item._id)}
                      className="p-2 text-neutral-400 hover:text-red-500 border dark:border-neutral-800 rounded-lg"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
        ...
        {activeDeletionTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isDeleting && setActiveDeletionTarget(null)}
            ...
            <motion.div
              ...
              className="relative w-full max-w-md rounded-2xl border border-neutral-200/80 bg-white p-6 shadow-xl dark:border-neutral-900 dark:bg-neutral-950 z-10 space-y-4"
            >
              <div className="flex items-center gap-3 text-amber-500">
                <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  <AlertTriangle size={20} />
                </div>
                <h4 className="text-lg font-bold tracking-tight text-neutral-900 dark:text-white">
                  Confirm Deletion
                </h4>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  onClick={() =>
                    confirmDestructiveRemoval(activeDeletionTarget)
                  }
                  disabled={isDeleting}
        ...
        {viewTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: 8 }}
              className="relative w-full max-w-lg rounded-2xl border border-neutral-200/80 bg-white p-6 shadow-xl dark:border-neutral-900 dark:bg-neutral-950 z-10 max-h-[85vh] overflow-y-auto"
            >
              <button
                onClick={() => setViewTarget(null)}
                className="absolute top-4 right-4 p-1.5 rounded-lg text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900"
              >
                <X size={16} />
              </button>
              ...
              <img
                ...
                className="w-full aspect-video object-cover rounded-xl mb-4 border dark:border-neutral-900"
              />
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-xl bg-neutral-50 dark:bg-neutral-900/40">
                  <p className="text-neutral-400 font-bold uppercase mb-0.5">
                    Price
                  </p>
                ...
                <div className="p-3 rounded-xl bg-neutral-50 dark:bg-neutral-900/40">
                  ...
                  <p
                    className={`font-bold ${viewTarget.availability ? "text-emerald-600" : "text-neutral-400"}`}
                  >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      >
        <button
          onClick={onClose}
          disabled={isSaving}
          className="absolute top-4 right-4 p-1.5 rounded-lg text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900 disabled:opacity-50"
        >
          <X size={16} />
        </button>
