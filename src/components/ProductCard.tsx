...
// 1. Define the missing interface directly to prevent compiler crash
...
export default function ProductCard({
  ...
  const description =
    product.description ||
    "Premium quality curated item, organically harvested and locally handled with care.";
  const location = product.location || "Dhaka, Bangladesh";
  const date = product.date || "Available Now";

  // Modal State Pipeline
  ...
  const openQuickViewModal = (e: React.MouseEvent) => {
    // Prevent the parent card card click events from colliding
    e.preventDefault();
    e.stopPropagation();
    ...
    // Map your card data model dynamically into the detailed Modal structure
    const detailedData: ProductDetail = {
      ...
      sku: `00${product.id}-h9`,
      category: product.category,
      tags: [product.category.toLowerCase().replace(" & ", "-"), "organic"],
      images: [product.image, product.image],
      specifications: [
        { key: "Origin", value: location },
        { key: "Availability", value: date },
      ],
    };

    setActiveProduct(detailedData);
    
    // Smooth high-end loading response delay
  ...
  return (
    <>
      <motion.div
        ...
        onBlur={onHoverEnd}
        onClick={openQuickViewModal} // Clicking the body of the card initiates the quick view modal
        tabIndex={0}
        role="button"
        ...
        className={`group relative overflow-hidden rounded-2xl border border-neutral-200/70 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900/60 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500 focus-within:ring-offset-2 outline-none cursor-pointer flex transition-all ${
          viewMode === "grid"
            ? "flex-col justify-between h-[440px]"
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              className={`mt-2.5 flex gap-0.5 ${viewMode === "grid" ? "justify-center" : "justify-start"}`}
            >
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
          ...
          {/* Action Slide-Over Details Button */}
          <AnimatePresence>
            {isHovered && (
                ...
                <motion.div
                  ...
                  className={`flex items-center justify-center gap-2 border border-white/20 bg-neutral-900/80 px-4 py-2 text-[11px] font-bold tracking-wider text-white shadow-xl backdrop-blur-md transition-colors hover:bg-emerald-600 dark:bg-neutral-100 dark:text-black dark:hover:bg-emerald-500 dark:hover:text-white ${
                    viewMode === "grid"
                      ? "absolute bottom-4 left-4 right-4 rounded-xl"
                      : "relative rounded-lg shrink-0 mt-3 sm:mt-0"
                  }`}
                >
                  <Eye size={13} />
                  SEE DETAILS
      ...
      {/* Render the single instance modal safely right outside the card div root */}
